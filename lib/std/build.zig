...
pub const LibExeObjStep = struct {
    ...
    fn make(step: *Step) !void {
        ...
        if (self.kind == Kind.Lib and self.linkage == .dynamic and self.version != null and self.target.wantSharedLibSymLinks()) {
...
test "Builder.dupePkg()" {
    ...
    std.testing.expect(dupe.path.path.ptr != pkg_top.path.path.ptr);
    std.testing.expect(dupe_deps[0].name.ptr != pkg_dep.name.ptr);
    std.testing.expect(dupe_deps[0].path.path.ptr != pkg_dep.path.path.ptr);
}
